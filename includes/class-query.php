	...
	if ( !class_exists('QUERY') ) {
		class QUERY {
			public function load_user_object($user_id) {
				...
				return $obj[0];
			}public function load_user_object1($user_id) {
				global $db;
			...
			public function do_user_directory() {
				...
				foreach ( $users as $user ) { ?>
					<div class="directory_item">
						<h3><a href="/t/profile-view.php?uid=<?php echo $user->ID; ?>"><?php echo $user->user_nicename; ?></a></h3>
			...
			public function do_friends_list1($friends_array) {
				foreach ( $friends_array as $friend_id ) {
					$users[] = $this->load_user_object($friend_id);
				}
			?>
              <h3><a href="">Pansul Bhatt</a></h3>
						<p>pansulbhatt47@gmail.com</p>
			<?php					
				foreach ( $users as $user ) {?>
					<div class="directory_item">
						<h3><a href=""><?php echo $user->user_nicename; ?></a></h3>
						<p><?php echo $user->user_email; ?></p>
					</div>
				<?php
				}
			...
			public function do_inbox($user_id) {
				...
				foreach ( $message_objects as $message ) {?>
					<div class="status_item">
						...
						<h3>From: <a href="/t/profile-view.php?uid=<?php echo $user->ID; ?>"><?php echo $user->user_nicename; ?></a></h3>
